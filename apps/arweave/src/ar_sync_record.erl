...
read_sync_records(StateDB, StoreID) ->
	%% Fable C3 (2026-07-05): boot path. On DB failure the safe
	%% option is not "assume empty" — a silently-empty sync record
	%% causes the node to re-sync everything, which on a full node
	%% means hours of unnecessary chunk fetch. Crash loudly so the
	%% supervisor bubbles the error up and the operator can inspect
	%% the underlying db_operation_failed record.
	{SyncRecordByID, SyncRecordByIDType} =
		case ar_kv:get(StateDB, ?SYNC_RECORDS_KEY) of
			...
			{ok, V} ->
				binary_to_term(V);
			{error, Reason} ->
				error({read_sync_records_failed, StateDB, StoreID, Reason})
		end,
...
replay_write_ahead_log(SyncRecordByID, SyncRecordByIDType, StateDB, StoreID) ->
	%% Fable C3 (2026-07-05): boot path, same rationale as
	%% read_sync_records above.
	WAL =
		case ar_kv:get(StateDB, ?WAL_COUNT_KEY) of
			...
			{error, Reason} ->
				error({replay_wal_count_failed, StateDB, StoreID, Reason})
		end,
...
replay_write_ahead_log(SyncRecordByID, SyncRecordByIDType, N, WAL, StateDB, StoreID, Module) ->
	case ar_kv:get(StateDB, binary:encode_unsigned(N)) of
		not_found ->
			...
			{SyncRecordByID, SyncRecordByIDType, WAL};
		%% Fable C3 (2026-07-05): boot-time WAL replay. Same as
		%% read_sync_records — a partial replay is worse than a
		%% loud crash because the resulting in-memory state
		%% silently misrepresents what is on disk.
		{error, Reason} ->
			error({replay_wal_entry_failed, StateDB, StoreID, N, Reason});
		{ok, V} ->
